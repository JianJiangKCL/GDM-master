...
class GammaGWR:
    ...
    #todo what is alpha_w
    # only used in depth coefficient, there is k context descriptor so, k alpha needed, deeper it is ,the smaller alpha is,
    # and their sum is 1
    ...
    def compute_distance(self, x, y) -> float:
        # l2-norm TE x^2
        # returning value is a scalar, so alphas and x,y should be vector.
        # np.dot(vectorA,vectorB) = E(xa * xb), can be regard as raw vec multipled by column vec
        # np.dot(matA,matB) = matC  mxn x nxz = mxz, matrix production
        # np.dot(A,scalar) = A ; every element of A times by the scalar
        return np.linalg.norm(np.dot(self.alphas.T, (x-y))) 

    def find_bs(self, dis) -> Tuple[int, float, int]:
        # Return a list with the n smallest elements from the dataset defined by iterable. ke
        bs = nsmallest(2, ((k, i) for i, k in enumerate(dis))) # i is idx, k is distance. so [0] is the distance, [1] is the idx
        # return the bmu idx, bmu distance, sec-bmu idx
    ...
    # padding the right and bottom with 0.
    ...
    def init_network(self, ds, random, **kwargs) -> None:
        ...
        self.num_context = kwargs.get('num_context', 0) # context descriptor
        self.depth = self.num_context + 1   # temporal depth
        empty_neuron = np.zeros((self.depth, self.dimension))   # depth x dim
        # ATT python list store the direct object not the copy.
        #TODO , weights should store two different neurons ,but now it store the same neural twice
        # seems like it would be replaced later
        self.weights = [empty_neuron, empty_neuron]
        
        # Global context
        self.g_context = np.zeros((self.depth, self.dimension)) # depth x dim, thought global context only one. but maybe this as a whole is the one.
        
        # Create habituation counters
        self.habn = [1, 1]  # hj
        
        # Create edge and age matrices
        self.edges = np.ones((self.num_nodes, self.num_nodes))  # connections between current existing nodes (neurons)
        self.ages = np.zeros((self.num_nodes, self.num_nodes))

        # Label histograms
        empty_label_hist = -np.ones(ds.num_classes)
        # the times a neuron selected as the BMU with label l.
        # ATT python list store the direct object not the copy.
        self.alabels = [empty_label_hist, empty_label_hist]
        
        # Initialize weights
        self.random = random
        if self.random: init_ind = np.random.randint(0, ds.vectors.shape[0], 2)  # randint(start,end,times) can be repeatitive; 2 is the num_nodes
    ...
    # update the weight of target neuron.
    def update_weight(self, index, epsilon) -> None:
        # dw = (x(t)-wi) * epsilon* hi
    ...
    def update_neighbors(self, index, epsilon) -> None:
        # np.nonzero(A) returns the indices of non-zero elements in A
    ...
    def update_labels(self, bmu, label, **kwargs) -> None:
        ...
        if not new_node:
            for a in range(0, self.num_classes):
                if a == label:
                    self.alabels[bmu][a] += self.a_inc  # a_inc = 1
                else:
                    if label != -1:
                        # forgetting the relation of a label
                        self.alabels[bmu][a] -= self.a_dec  #a_dec = 0.1
                        # non-negative relation.
                        if (self.alabels[bmu][a] < 0):
                            self.alabels[bmu][a] = 0
        else:
            # a new neuron with all zeros relation with labels
